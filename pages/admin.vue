<template>
  <div class="min-h-screen bg-gradient-to-b from-[#1A043C] to-[#371859] flex flex-col">
          ...
          <div class="flex items-center space-x-4">
            <div class="flex-shrink-0">
              <NuxtLink to="/" class="block hover:opacity-80 transition-opacity bg-transparent">
                <img src="/white-logo.png?v=3" alt="Logo" class="h-16 w-16 object-contain bg-transparent" />
              </NuxtLink>
            </div>
            <div>
      ...
      <!-- Contenido principal -->
        ...
        <div class="bg-white/0 backdrop-blur-sm rounded-lg border border-white/20">
          ...
          <div v-if="filteredPosts && filteredPosts.length > 0" class="divide-y divide-white/20 overflow-x-auto min-w-full">
            <div v-for="post in filteredPosts" :key="post.id" class="px-4 sm:px-6 py-4">
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-gray-900 rounded-lg border border-gray-700 p-4 sm:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
...
const getFilteredPostsTitle = () => {
  switch (activeFilter.value) {
    case 'published':
      return 'Publicaciones Publicadas'
    case 'draft':
      return 'Borradores'
    case 'RESOLUCIÓN':
      return 'Resoluciones'
...
const getNoPostsMessage = () => {
  switch (activeFilter.value) {
    case 'published':
      return 'No hay publicaciones publicadas.'
    case 'draft':
      return 'No hay borradores disponibles.'
    case 'RESOLUCIÓN':
      return 'No hay resoluciones disponibles.'
    case 'COMUNICADO':
      return 'No hay comunicados disponibles.'
    default:
